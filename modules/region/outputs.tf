  ...
  value = {
    ...
    env => {
      ...
      zone_key => {
        ...
        globals          = var.region.globals # direct reference, not merged
        template_version = var.region.template_version
        regional = {
          gcp_region      = var.region.gcp_region # direct reference
          proxy_only_cidr = var.proxy_only_cidr   # direct reference
        }
      }
    }
  }
